...
| `import { FastCheck } from "effect"`   | `import * as fc from "fast-check"` (not re-exported) |
...
## Scoped resources are finalized per test

`it.effect` and `it.live` already provide a `Scope` and close it when the test completes, so `Effect.acquireRelease`,
`Stream.scoped`, and resourceful layers need no extra wrapper. v3's `it.scoped` / `it.scopedLive` are gone.
...
- Needs wall clock, Node timers, or real delays? Use `it.live`.
- Allocates resources that must be finalized? `it.effect` and `it.live` are already scoped.
