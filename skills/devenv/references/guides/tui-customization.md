...
`shell.prompt_prefix` controls the `(devenv)` prefix in interactive shell prompts and defaults to `true`. Disable it to keep your own prompt, for example when using Starship:
...
A project’s `prompt_prefix` setting in `devenv.yaml` or `devenv.local.yaml` takes precedence. Changes take effect when you start a new shell.
