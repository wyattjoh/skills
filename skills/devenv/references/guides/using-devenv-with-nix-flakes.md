...
> **For those new to devenv and Nix**
>
...
Here’s a minimal `flake.nix` to start you off that includes a `devShell` created with `devenv.lib.mkShell`. See [the reference documentation](/reference/options/) for the possible options to use here.
...
> **running tests with flakes doesn’t support starting processes, for that you need to use `devenv`**
