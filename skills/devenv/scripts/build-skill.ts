...
const TREE_URL = "https://api.github.com/repos/cachix/devenv/git/trees/main?recursive=1";
...
const orderKey = (p: string) => p.replace(/(^|\/)index\.mdx?$/, "$1");
...
function orderedDocPaths(treeJson: string): string[] {
  ...
  return tree.tree
    ...
    .toSorted((a, b) => (orderKey(a) < orderKey(b) ? -1 : orderKey(a) > orderKey(b) ? 1 : 0));
}
...
function buildSections(lines: string[], heads: Head[]): Sec[] {
  const isStub = (h: Head, next?: Head) => {
    ...
    const body = lines.slice(h.line, next.line - 1).filter((l) => l.trim() !== "");
