...
state through the documented registry and schema-1 run-state contract.
...
- A folder (`.scratch/<folder>`, default `.scratch/core-coordinator`) holds
  `brief.md` and your own `RESUME.md`.
- `resume .scratch/<folder>` is the successor form after a handoff.

## Boundaries

The canonical registry is the integration checkout's
`.scratch/coordinators.md`. Consume the schema in
[coordinate-implementation's registry contract](../coordinate-implementation/references/registry.md).
For each row, read the referenced schema-1 RESUME.md and its `## Run outcome`.
Reject unsupported schema versions without migration.

Each run's current ready coordinator exclusively owns its registry row and
RESUME.md. You own only your RESUME.md and the registry's
`## Cross-run agreements` section.

**Never edit a run's `RESUME.md` or its registry row.** Report drift to the run
owner and wait for that owner to update both records. A `waiting` run is
...
2. Read `brief.md`, your RESUME.md when present, the canonical registry, every
   referenced run RESUME.md, and every completed SUMMARY.md.
3. Validate each registry row against its run prefix, base, ownership
   generation, ready pane, run status, and active tickets. Refresh compacted
   pane ids through Herdr, but ask the run owner to persist corrections.
...
6. **Completion.** Read terminal SUMMARY.md, capture landed and closed work,
   retained branches, review provenance, and pending tracker action. Acknowledge
   the completion tombstone so the run owner may remove its registry row.

## Event-driven cycle

Follow [loop.md](references/loop.md). React to Herdr status events,
cross-session messages, registry changes, and bounded timeout snapshots. Do not
...
pending decisions, registry observations, and acknowledgements first. Discover
...
1. Read `brief.md`, your RESUME.md, the canonical registry, and every referenced
   run state before messaging anyone.
2. Refresh Herdr pane identities from the machine-readable control surface.
3. Revalidate row ownership and run outcome fields without modifying run-owned
   records.
...
- Never push, write to trackers, touch remotes, delete branches, or remove
  worktrees.
- Never edit run-owned state or registry rows.
